...
import net.perkowitz.issho.models.*;
import net.perkowitz.issho.models.Track;
...
import static net.perkowitz.issho.SequencerInterface.ValueMode.FILL_PERCENT;
import static net.perkowitz.issho.SequencerInterface.ValueMode.TEMPO;
import static net.perkowitz.issho.SequencerInterface.ValueMode.VELOCITY;
...
//        System.out.printf("selectStep: %d, %s\n", index, stepMode);
        ...
        if (stepMode == StepMode.MUTE) {
            ...
            nextStepIndex = (index + net.perkowitz.issho.models.Track.getStepCount()) % net.perkowitz.issho.models.Track.getStepCount();
        } else if (stepMode == StepMode.VELOCITY) {
            memory.select(step);
            valueMode = VELOCITY;
            display.displayValue(step.getVelocity(), VELOCITY_MIN, VELOCITY_MAX, ValueMode.VELOCITY);
        } else if (stepMode == StepMode.PLAY) {
            net.perkowitz.issho.models.Track track = memory.selectedPattern().getTrack(index);
...
//            nextPattern(patternChain.get(0));
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
            public void run() {
                if (playing && memory.isSet(Switch.INTERNAL_CLOCK_ENABLED)) {
                    ...
                    if (totalStepCount % net.perkowitz.issho.models.Track.getStepCount() == 0) {
                        andReset = true;
                    }
                    advance(andReset);
                }
    ...
    private void advance(boolean andReset) {
        ...
        // send the midi notes
        for (net.perkowitz.issho.models.Track track : memory.playingPattern().getTracks()) {
